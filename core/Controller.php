...
//mod
	...
	public function setConf()
	{
		...
		foreach (explode(",", MODULES) as $i) {
            ...
            $sidebar = $this->{$i}->sidebar();
            $this->sidebar->{$i} = !empty($sidebar) ? $sidebar : array();
		}
        ...
        $this->view->setVar('sidebar_arr', $this->sidebar);
        $this->view->setVar('sidebar', 'user2.tpl.php');
    ...
    public function check_tmpl($tmpl, $ext)
    {
		if (file_exists(TEMPLATE_PATH.$tmpl.$ext)){
            file_exists('./templates/Default/'.$tmpl.$ext);
    ...
    public function token()
    {
		...
		{
            $last = $_SESSION['token'][1];
            $_SESSION['token'] = null;
			$_SESSION['token'][0] = $last;
		}
